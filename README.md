...
AutoTask 是一个远程 GPU 监控与任务启动工具。它可以读取 SSH 服务器的 GPU 状态，也可以把远端 `.sh` 脚本放进 `tmux` 后台运行。
...
AutoTask 不保存密码，也不处理交互式密码登录；请先配置好 SSH 密钥或免密登录。
...
Monitor 用来看远程 GPU 状态。打开后会先显示上次缓存结果，再在后台刷新；之后点击 **刷新** 或单台服务器的 **刷新** 才会再次访问远程服务器。
...
- 找不到 Conda：先安装 Anaconda 或 Miniconda，再重新运行对应系统的 `setup` 文件。
- SSH 失败：先在本机终端确认 `ssh <Host>` 可以成功登录。

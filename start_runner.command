...
clear
printf 'AutoTask4macOS Runner 启动器\n'
printf '这个窗口保持打开时，任务管理后台正在运行。关闭窗口不会停止远端 tmux 任务。\n'

progress "1/3 查找 Conda"
CONDA_BIN="$(find_conda || true)"
if [ -z "$CONDA_BIN" ]; then
  log "没有找到 Conda。请先安装 Anaconda 或 Miniconda，然后重新双击 start_runner.command。"
  osascript -e 'display dialog "没有找到 Conda。请先安装 Anaconda 或 Miniconda，然后重新双击 start_runner.command。" buttons {"好"} default button "好"' >/dev/null 2>&1
  if [ -t 0 ]; then
    printf '\n启动失败。按回车关闭这个窗口。'
    read -r _
  fi
...
if ! "$CONDA_BIN" env list | awk '{print $1}' | grep -qx "${ENV_NAME}"; then
  ...
  osascript -e 'display dialog "AutoTask4macOS 还没有完成首次环境配置。请先双击 setup.command，完成后再双击 start_runner.command。" buttons {"好"} default button "好"' >/dev/null 2>&1
  if [ -t 0 ]; then
...
progress "3/3 启动任务页面"
log "正在启动 AutoTask4macOS Runner。浏览器会自动打开网页。"
